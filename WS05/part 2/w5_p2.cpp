...
int main(int argc, char** argv)
{
    ...
    if (argc == 5) 
    {
        ...
        std::string line;
        size_t count = 0;
        while (count < 4 && std::getline(fileStream, line)) 
        {
            if (line[0] != '#') 
            {
    ...
    auto exchangeRate = [=](seneca::Book& book) 
    {
        if (book.country() == "US") 
        {
  ...
  std::cout << "-----------------------------------------\n";
    ...
    if (argc > 2) 
    {
        ...
        while (count < 5 && std::getline(movieFile, line)) 
        {
            ...
            {
                seneca::Movie tempMovie(line);
                movies[count++] = tempMovie;
            }
        }
    }
    ...
    try 
    {
        ...
        std::cout << theCollection[theCollection.size()];
    } 
   ...
   std::cout << "-----------------------------------------\n\n";
    ...
    for (auto i = 3; i < argc; ++i)
    {
        ...
        try
        {
                seneca::SpellChecker spellChecker(argv[i]);
            for (size_t i = 0; i < library.size(); ++i) 
            {
                library[i].fixSpelling(spellChecker);
            }
                spellChecker.showStatistics(std::cout);
            for (size_t i = 0; i < theCollection.size(); ++i) 
            {
                theCollection[i].fixSpelling(spellChecker);
            }
                spellChecker.showStatistics(std::cout);
        }
